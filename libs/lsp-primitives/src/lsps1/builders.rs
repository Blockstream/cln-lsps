...
impl Lsps1InfoResponseBuilder {
    ...
    pub fn build(self) -> Result<Lsps1InfoResponse> {
        // Required fields
        let supported_versions = self.supported_versions.context("Missing field 'supported_versions' in Lsps1InfoResponseBuilder")?;
        let options = self.options.context("Missing field 'options' in Lsps1InfoResponseBuilder")?;
...
impl Lsps1OptionsBuilder {
    ...
    pub fn minimum_onchain_payment_confirmations(mut self, minimum_onchain_payment_confirmations : u8) -> Self {
        self.minimum_onchain_payment_confirmations = Some(minimum_onchain_payment_confirmations);
        self
    }
    ...
    pub fn min_onchain_payment_size_sat(mut self, min_onchain_payment_size_sat : u64) -> Self {
        self.min_onchain_payment_size_sat = Some(min_onchain_payment_size_sat);
        self
    }
    ...
    pub fn min_initial_client_balance_sat(mut self, min_initial_client_balance_sat : SatAmount) -> Self {
    ...
    pub fn max_initial_client_balance_sat(mut self, max_initial_client_balance_sat : SatAmount) -> Self {
    ...
    pub fn build(self) -> Result<Lsps1Options> {
        ...
        let max_initial_lsp_balance_sat = self.max_initial_lsp_balance_sat.context("No value specified for 'max_initial_lsp_balance_sat' in Lsps1OptionsBuilder")?;

        let min_initial_client_balance_sat = self.min_initial_client_balance_sat.context("No value specified for 'min_initial_client_balance_sat' in Lsps1OptionsBuilder")?;
        let max_initial_client_balance_sat = self.max_initial_client_balance_sat.context("No value specified for 'max_initial_client_balance_sat' in Lsps1OptionsBuilder")?;
        let min_channel_balance_sat = self.min_channel_balance_sat.context("No value specified for 'min_channel_balance_sat' in Lsps1OptionsBuilder")?;
        let max_channel_balance_sat = self.max_channel_balance_sat.context("No value specified for 'max_channel_balance_sat' in Lsps1OptionsBuilder")?;
        ...
        if ! (min_initial_client_balance_sat <= max_initial_client_balance_sat) {
            return Err(anyhow!("min_initial_client_balance_sat ({}) should be less than or equal to max_initial_client_balance_sat ({})", min_channel_balance_sat, max_channel_balance_sat))
        }
        if ! (min_channel_balance_sat <= max_channel_balance_sat) {
            return Err(anyhow!("min_channel_balance_sat ({}) should be less than or equal to max_channel_balance_sat ({})", min_channel_balance_sat, max_channel_balance_sat))
        }
...
impl Lsps1GetOrderRequestBuilder {
    ...
    pub fn build(self) -> Result<Lsps1GetOrderRequest> {
        // Required fields
        let api_version = self.api_version.context("Missing field 'api_version' in Lsps1GetOrderRequestBuilder")?;
        let lsp_balance_sat = self.lsp_balance_sat.context("Missing field 'lsp_balance_sat' in Lsps1GetOrderRequestBuilder")?;
        let client_balance_sat = self.client_balance_sat.context("Missing field 'client_balance_sat' in Lsps1GetOrderRequestBuilder")?;
        let confirms_within_blocks = self.confirms_within_blocks.context("Missing field 'confirms_within_blocks' in Lsps1GetOrderRequestBuilder")?;
        let channel_expiry_blocks = self.channel_expiry_blocks.context("Missing field 'channel_expirty_blocks' in Lsps1GetOrderRequestBuilder")?;
        let announce_channel = self.announce_channel.context("Missing field 'announce_channel' in Lsps1GetOrderRequestBuilder")?;
...
impl Lsps1GetOrderResponseBuilder {
    ...
    pub fn build(self) -> Result<Lsps1GetOrderResponse> {
        //required variables
        let order_id = self.uuid.context("Missing field 'order_id' in Lsps1GetOrderRequestBuilder")?;
        let api_version = self.api_version.context("Missing field 'api_version' in Lsps1GetOrderRequestBuilder")?;
        let lsp_balance_sat = self.lsp_balance_sat.context("Missing field 'lsp_balance_sat' in Lsps1GetOrderRequestBuilder")?;
        let client_balance_sat = self.client_balance_sat.context("Missing field 'client_balance_sat' in Lsps1GetOrderRequestBuilder")?;
        let confirms_within_blocks = self.confirms_within_blocks.context("Missing field 'confirms_within_blocks' in Lsps1GetOrderRequestBuilder")?;
        let channel_expiry_blocks = self.channel_expiry_blocks.context("Missing field 'channel_expiry_blocks' in Lsps1GetOrderRequestBuilder")?;
        let token = self.token.context("Missing field 'token' in Lsps1GetOrderRequestBuilder")?;
        let announce_channel = self.announce_channel.context("Missing field 'announce_channel' in Lsps1GetOrderRequestBuilder")?;
        let created_at = self.created_at.context("Missing field 'created_at' in Lsps1GetOrderRequestBuilder")?;
        let order_state = self.order_state.context("Missing field 'order_state' in Lsps1GetOrderRequestBuilder")?;
        let expires_at = self.expires_at.context("Missing field 'expires_at' in Lsps1GetOrderRequestBuilder")?;
        let payment = self.payment.context("Missing field 'payment' in Lsps1GetOrderRequestBuilder")?;
...
impl PaymentBuilder {
    ...
    pub fn required_onchain_block_confirmations(mut self, required_onchain_block_confirmations : u8) -> Self {
    ...
    pub fn build(self) -> Result<Payment> {
        ...
        let fee_total_sat = self.fee_total_sat.context("Missing field 'fee_total_sat'")?; 
        let order_total_sat = self.order_total_sat.context("Missing field 'order_total_sat'")?; 
        let bolt11_invoice = self.bolt11_invoice.context("Missing field 'bolt11_invoice'")?; 
        ...
        if onchain_address.is_none() {
            ...
            if minimum_fee_for_0conf.is_some() {
                return Err(anyhow!("'minimum_fee_for_0conf' expected to be null because 'onchain_address' is null"))
            }
        }

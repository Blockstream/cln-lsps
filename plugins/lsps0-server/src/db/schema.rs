...
use lsp_primitives::lsps0::common_schemas::{FeeRate, IsoDatetime, PublicKey, SatAmount, NetworkChecked};
...
impl Lsps1OrderBuilder {
   ...
   pub fn order_request(mut self, order_request : &Lsps1CreateOrderRequest<NetworkChecked>) -> Self {
       ...
       self.refund_onchain_address = order_request.refund_onchain_address.clone().map(|address| address.to_string());
   ...
   pub fn build(self) -> Result<Lsps1Order> {
       ...
       let client_balance_sat = self.client_balance_sat.context("Missing field 'client_balance_dat'")?;
       let lsp_balance_sat = self.lsp_balance_sat.context("Missing field 'lsp_balance_sat'")?;
       let client_node_id = self.client_node_id.context("Missing field 'client_node_id'")?;
       let channel_expiry_blocks = self.channel_expiry_blocks.context("Missing field 'channel_expiry_blocks'")?;
       let confirms_within_blocks = self.confirms_within_blocks.context("Missing field 'confirms_within_blocks'")?;
       ...
       let announce_channel = self.announce_channel.context("Missing field 'announce_channel'")?;
...
impl Lsps1PaymentDetailsBuilder {
    ...
    pub fn onchain_block_confirmations_required(mut self, onchain_block_confirmations_required : Option<u8>) -> Self {
    ...
    pub fn build(self) -> Result<Lsps1PaymentDetails> {
        Ok(Lsps1PaymentDetails {
            fee_total_sat : self.fee_total_sat.context("Missing field 'fee_total_sat'")?,
            order_total_sat : self.order_total_sat.context("Missing field 'order_total_sat'")?,
            bolt11_invoice : self.bolt11_invoice.context("Missing field 'bolt11_invoice'")?,

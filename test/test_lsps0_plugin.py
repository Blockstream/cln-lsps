...
def get_plugin_dir_lsps0_dummy_server() -> str:
    cwd = os.getcwd()
    return os.environ.get("LSPS0_CLIENT_PATH", os.path.join(cwd, "build/plugins/lsps0-server"))
...
def test_lsps0_client_can_find_servers(node_factory : NodeFactory):
    ...
    lsp1 : LightningNode = node_factory.get_node(options={"plugin-dir" : plugin_dummy_server})
    lsp2 : LightningNode = node_factory.get_node(options={"plugin-dir" : plugin_dummy_server})
    ...
    lsp_client : LightningNode = node_factory.get_node(options={"plugin-dir" : plugin_lsps0_client})
